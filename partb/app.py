...
from partb.routers.share_router import router as share_router
...
async def serve_shared(share_token: str):
    html_path = FRONTEND_DIR / "pages" / "shared.html"
    if html_path.is_file():
        return HTMLResponse(content=html_path.read_text(encoding="utf-8"))
    return HTMLResponse("<h2>frontend/pages/shared.html not found</h2>", status_code=404)