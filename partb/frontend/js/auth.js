(function () {
  ...
  function saveAuth(store, token, user) {
    store.setItem("krutrim-auth", "true");
    store.setItem("kr_token", token);
    store.setItem("kr_user", JSON.stringify(user));
  }
  ...
  document.querySelectorAll('.pw-toggle').forEach(function (btn) {
    btn.addEventListener('click', function () {
      ...
      btn.innerHTML = show
        ? '<svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M1 12s4-8 11-8 11 8 11 8-4 8-11 8-11-8-11-8z"></path><line x1="1" y1="1" x2="23" y2="23"></line><circle cx="12" cy="12" r="3"></circle></svg>'
  ...
  if (loginForm) {
    loginForm.addEventListener('submit', async function (e) {
      ...
      try {
        ...
        saveAuth(remember ? localStorage : sessionStorage, data.token, {
  ...
  if (signupForm) {
    signupForm.addEventListener('submit', async function (e) {
      ...
      try {
        ...
        if(loginRes.ok) {
            saveAuth(sessionStorage, loginData.token, {
