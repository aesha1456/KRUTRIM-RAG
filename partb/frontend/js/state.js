...
export function isAuthed() {
  return !!getAuth('krutrim-auth');
}

export function saveAuth(key, value, remember) {
  (remember === false ? sessionStorage : localStorage).setItem(key, value);
}
...
export async function apiFetch(path, opts = {}) {
  ...
  if (res.status === 401) {
    ...
    sessionStorage.removeItem("krutrim-auth");
    sessionStorage.removeItem("kr_token");
