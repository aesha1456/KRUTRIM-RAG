...
def async_time_it(func: Callable) -> Callable:
    ...
    Supports both regular async functions (awaited) and async generators
    (iterated with async for).  For generators, timing is measured from
    construction to final iteration-complete."""
    _internal = func.__name__.startswith("_")
    _name = _label(func)
