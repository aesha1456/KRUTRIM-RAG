...
def get_qdrant():
    ...
    if _qdrant_cl is None:
        ...
        _qdrant_cl = QdrantClient(url=QDRANT_URL, api_key=QDRANT_API_KEY or None)
    return _qdrant_cl
...
def get_nomic():
    ...
    if _nomic_model is None:
        ...
        _nomic_model = SentenceTransformer(
            str(model_dir), trust_remote_code=True, device=DEVICE
        )
    return _nomic_model
        ...
        results.append(
            {
                ...
                "element_ids": pl.get("element_ids") or [],
                "bounding_boxes": pl.get("bounding_boxes") or [],
                "score": h.score,
            }
        )
    return results
