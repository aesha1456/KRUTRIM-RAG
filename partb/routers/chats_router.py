...
@time_it
def share_chat(chat_id: str, message_id: str | None = None, user: dict = Depends(verify_token)):
    chat = _assert_owner(chat_id, user["user_id"])
    if chat.get("share_token"):
        if message_id:
            ids = set(chat.get("shared_message_ids", []))
            ids.add(message_id)
            chats_col().update_one(
                {"chat_id": chat_id},
                {"$set": {"shared_message_ids": list(ids)}},
            )
        return {"share_token": chat["share_token"], "share_url": f"/shared/{chat['share_token']}"}
    token = str(uuid.uuid4())
    updates = {"share_token": token, "shared_at": _now_iso()}
    if message_id:
        updates["shared_message_ids"] = [message_id]
    chats_col().update_one(
        {"chat_id": chat_id},
        {"$set": updates},
    )
...
@time_it
def unshare_chat(chat_id: str, user: dict = Depends(verify_token)):
    _assert_owner(chat_id, user["user_id"])
    chats_col().update_one(
        {"chat_id": chat_id},
        {"$unset": {"share_token": "", "shared_at": "", "shared_message_ids": ""}},
    )
            ...
            async for event in run_rag_stream(
                ...
                if event.get("type") == "done":
                    ...
                    if final_text:
                        ...
                        event["message_id"] = doc.get("message_id")
                else:
                    yield _sse(event)
