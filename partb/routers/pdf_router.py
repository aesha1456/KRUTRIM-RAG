...
@router.get("/{book_id}")
@async_time_it
async def serve_pdf(book_id: str, token: str = Query(None), share: str = Query(None)):
    """
    ...
    Also accepts ?share={share_token} for public shared chats.
    """
    if share:
        from partb.config import MONGO_DB
        from partb.db import get_mongo
        chat = get_mongo()[MONGO_DB]["chats"].find_one({"share_token": share})
        if not chat:
            raise HTTPException(401, "Invalid or expired share link")
    elif token:
        try:
