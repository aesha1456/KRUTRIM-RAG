...
from workers.config import HEARTBEAT_INTERVAL_SECONDS, RESULT_CACHE_DIR, SERVER_URL
...
_cache = ResultCache("qdrant", base_dir=RESULT_CACHE_DIR)
...
def start_worker():
    ...
    This must not run while the module is being imported. Qdrant's native
    upload_points(parallel=...) uses multiprocessing; on Windows those child
    processes import this module again with the ``spawn`` start method. Keeping
    all worker runtime code here prevents an uploader child from becoming a
    second polling worker during its bootstrap phase.
    """
    is_connected = False
    ...
    while True:
        ...
        try:
            ...
            # Download only the checkpoint needed by this batch. The old worker
            # downloaded both large JSON files for every proposition/section job.
            if batch_kind == "sections":
            ...
            accepted = submit_with_retry(
                _session, f"{SERVER_URL}/submit_qdrant_result", payload
            )
            if accepted:
                _cache.clear(job_id)
                ...
                logger.warning(
                    "Qdrant result for batch #%s was not accepted; keeping local cache for replay",
                    batch_idx,
                )
