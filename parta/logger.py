...
logger.py — Part A
==================
Provides timing decorators and a structured logger for the RAG pipeline.
...
• @log_process / @async_log_process → always logs START + DONE/FAILED with total
  time, regardless of function name or duration.  Use on major pipeline stages.
...
# ─── Logger ──────────────────────────────────────────────────────────────────
logger = logging.getLogger("RAG.parta")
...
def log_process(func: Callable) -> Callable:
    """
    Decorator for major pipeline stages.
