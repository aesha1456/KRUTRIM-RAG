...
import os
from pathlib import Path
from partb.config import NEO4J_DATABASE, NEO4J_PASSWORD, NEO4J_URI, NEO4J_USER
from workers.config import NEO4J_CONNECTION_POOL_SIZE, NEO4J_LOCAL_WORKERS
...
def _load_gliner(base_dir: Path):
    ...
    logger.info(f"[NEO4J] Loading GLiNER from {model_dir} on {DEVICE}...")
    t0 = time.time()
...
def _process_chunks(
    ...
    args_iter = [
        (idx, chunk, book_id, gliner_model, sent_tokenize)
        for idx, chunk in enumerate(chunks)
    ]
    done = 0

    # Keep Neo4j writes in this thread, but optionally parallelize the CPU-side
    # extraction.  The default is one model call stream because concurrent
    # inference on a shared GPU model often reduces throughput; set
    # NEO4J_LOCAL_WORKERS=2 on CPU-heavy office machines after benchmarking.
    if NEO4J_LOCAL_WORKERS == 1:
        results = (_neo4j_chunk_worker(args) for args in args_iter)
        for result in results:
            handle_result(result)
            done += 1
            if done == total or (done % 25 == 0 and done != total):
                report(done)
    else:
        with concurrent.futures.ThreadPoolExecutor(
            max_workers=NEO4J_LOCAL_WORKERS,
            thread_name_prefix="neo4j-extract",
        ) as pool:
            futures = [pool.submit(_neo4j_chunk_worker, args) for args in args_iter]
...
def run_neo4j_batch(
    ...
    If *driver* is provided the caller owns it (create once, reuse across calls).
    Otherwise a new driver is created and closed inside this function.
    ...
    if driver is None:
        ...
        logger.info("[NEO4J-BATCH] Reusing persistent driver.")
