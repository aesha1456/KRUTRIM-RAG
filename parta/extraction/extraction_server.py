...
async def lifespan(_app: FastAPI):
    """Start and stop the lease reaper with the FastAPI application."""
    _start_lease_reaper()
    try:
        yield
    finally:
...
app = FastAPI(title="Unified Job Server", lifespan=lifespan)
...
def _finish_check(store: Dict[str, dict], book_id: str, cleanup_fn=None):
    """Finish only when every actual job is terminal.

    The old counter-only check could finish a book when a late duplicate
    result incremented completed/failed twice. Statuses are authoritative.
    """
    ...
    if (
        len(state["jobs"]) == state["total"]
        and all(job["status"] in terminal for job in state["jobs"].values())
...
def _new_extraction_chunk_dir(base_dir: str, book_id: str) -> Path:
    """Return a unique temporary directory for one extraction generation."""
    extraction_id = uuid.uuid4().hex[:12]
    return Path(base_dir) / f"{TEMP_EXTRACT_DIR_PREFIX}_{book_id}_{extraction_id}"
...
def _touch_worker(worker_id: str, worker_type: str, worker_ip: str,
                  status: str = "idle", current_job: Optional[str] = None) -> None:
    if not worker_id or worker_id == "unknown":
        return
    row = worker_registry.setdefault(worker_id, {
        "worker_id": worker_id,
    ...
    row.update({
        "worker_type": worker_type,
        "ip": worker_ip if worker_ip != "unknown" else row.get("ip", "unknown"),
        "status": status,
        "current_job": current_job,
        "last_seen": time.time(),
    })
...
def _requeue_expired_jobs(
    ...
    """Return expired leases to the queue exactly once.

    This function is called by both the polling path and the independent
    reaper. It is always invoked while ``job_lock`` is held by the caller.
    """
    ...
    for book_id, state in store.items():
        ...
        for job in state["jobs"].values():
            if job["status"] != "PROCESSING" or now <= job["lease_deadline"]:
                continue

            old_worker = job.get("assigned_to")
            job["attempt_count"] += 1
            ...
            logger.warning(
                ...
                job["job_kind"].upper(), job["job_id"], old_worker,
                job["attempt_count"], MAX_ATTEMPTS,
            )

            if job["attempt_count"] >= MAX_ATTEMPTS and not _should_use_normal_fallback(job):
                job["status"] = "FAILED"
                state["failed"] += 1
                job["error"] = "Lease expired too many times"
                logger.error(
                    "%s job %s permanently failed after %d attempts",
                    job["job_kind"].upper(), job["job_id"], MAX_ATTEMPTS,
                )
                _finish_check(store, book_id, cleanup_fn=cleanup_fn)
            else:
                if _should_use_normal_fallback(job):
                    job["fallback_attempted"] = True
                    job["error"] = "Normal attempts exhausted; trying OpenDataLoader fallback"
                job["status"] = "PENDING"
                job["retry_at"] = now + _retry_delay(job["attempt_count"])
                if job["job_id"] in state["queue"]:
                    state["queue"].remove(job["job_id"])
                    state["queue"].append(job["job_id"])
                rescued += 1
    return rescued


def _should_use_normal_fallback(job: dict) -> bool:
    return bool(
        job["job_kind"] == "extraction"
        and job.get("ocr_enabled", False)
        and not job.get("fallback_attempted", False)
        and job.get("attempt_count", 0) >= MAX_ATTEMPTS
    )


def _retry_delay(attempt_count: int) -> float:
    return min(
        RETRY_BASE_DELAY_SECONDS * (2 ** max(attempt_count - 1, 0)),
        RETRY_MAX_DELAY_SECONDS,
    )
...
def _assign_pending_job(
    *,
    ...
    now = time.time()
    _touch_worker(worker_id, expected_kind or "unknown", worker_ip)
    _requeue_expired_jobs(
        store,
        now,
        cleanup_fn=_cleanup_after_delay if store is extractions else None,
    )
    ...
    for book_id, state in store.items():
        ...
        for jid in queue:
            ...
            if expected_kind and job["job_kind"] != expected_kind:
                continue
            if job["status"] != "PENDING" or job.get("retry_at", 0) > now:
                continue
            ...
            job["retry_at"] = 0
            _touch_worker(worker_id, job["job_kind"], worker_ip, "busy", jid)
            ...
            response = {
                ...
                "fallback_mode": bool(job.get("fallback_attempted", False)),
            }
    ...
    *,
    ...
    with job_lock:
        for book_id, state in store.items():
            ...
            _touch_worker(worker_id, job["job_kind"], "unknown", "idle", None)
            ...
            if job["status"] in {"COMPLETED", "FAILED"}:
                return {
                    ...
                    "accepted": False,
                    "terminal": True,
                    "note": "terminal result already recorded, ignored",
                }

            if job.get("assigned_to") != worker_id or job.get("lease_id") != lease_id:
                # Never accept a result from an old lease. Another worker may
                # already be processing this job, and accepting the stale
                # result could finalize the job before the current owner has
                # finished or repeat non-idempotent side effects.
                return {
                    "status": "ok",
                    "accepted": False,
                    "terminal": False,
                    "note": "stale lease result ignored",
                }
            ...
            if success and (content or not require_content):
                ...
                if job["attempt_count"] >= MAX_ATTEMPTS and not _should_use_normal_fallback(job):
                    ...
                    if _should_use_normal_fallback(job):
                        job["fallback_attempted"] = True
                        job["error"] = (
                            "Normal attempts exhausted; trying OpenDataLoader fallback"
                        )
                    job["status"] = "PENDING"
                    job["assigned_to"] = None
                    job["assigned_at"] = 0
                    job["lease_deadline"] = 0
                    job["lease_id"] = None
                    job["retry_at"] = time.time() + _retry_delay(job["attempt_count"])
                    # Rotate the failed job behind currently queued work.
                    if jid in state["queue"]:
                        state["queue"].remove(jid)
...
def heartbeat(payload: dict, request: Request):
    """Renew a processing lease so slow database/model jobs are not duplicated."""
    jid = payload.get("job_id")
    worker_id = payload.get("worker_id", "unknown")
    lease_id = payload.get("lease_id")
    if not jid or not worker_id or not lease_id:
        raise HTTPException(400, "job_id, worker_id and lease_id are required")

    with job_lock:
        for store in (extractions, neo4j_jobs, qdrant_jobs):
            for book_id, state in store.items():
                ...
                if (
                    job["status"] != "PROCESSING"
                    or job.get("assigned_to") != worker_id
                    or job.get("lease_id") != lease_id
                ):
                    raise HTTPException(409, "Job is no longer assigned to this worker")
                job["lease_deadline"] = time.time() + LEASE_SECONDS
                _touch_worker(worker_id, job["job_kind"], _client_ip(request), "busy", jid)
                return {"status": "ok", "lease_seconds": LEASE_SECONDS}

    raise HTTPException(404, f"Job {jid} not found")


def _lease_reaper_loop() -> None:
    """Recover dead workers without depending on another worker polling."""
    while not _lease_reaper_stop.is_set():
        try:
            with job_lock:
                for store in (extractions, neo4j_jobs, qdrant_jobs):
                    _requeue_expired_jobs(
        ...
        _lease_reaper_stop.wait(LEASE_REAPER_INTERVAL_SECONDS)


_lease_reaper_stop = threading.Event()
_lease_reaper_thread = None


def _start_lease_reaper() -> None:
    global _lease_reaper_thread
    if _lease_reaper_thread and _lease_reaper_thread.is_alive():
        return
    _lease_reaper_stop.clear()
    _lease_reaper_thread = threading.Thread(
        target=_lease_reaper_loop,
        name="job-lease-reaper",
        daemon=True,
    )
    _lease_reaper_thread.start()


def _stop_lease_reaper() -> None:
    global _lease_reaper_thread
    _lease_reaper_stop.set()
    if _lease_reaper_thread and _lease_reaper_thread is not threading.current_thread():
        _lease_reaper_thread.join(timeout=2)
    _lease_reaper_thread = None


@app.get("/workers")
def workers_status():
    """Return the live office-worker registry for operations monitoring."""
    now = time.time()
    with job_lock:
        rows = []
        for row in worker_registry.values():
            item = dict(row)
            item["online"] = now - item["last_seen"] <= HEARTBEAT_INTERVAL_SECONDS * 3
            rows.append(item)
...
def _cleanup_after_delay(meta: dict):
    """Delete one finished extraction generation without racing a replacement.

    Cleanup and replacement installation both synchronize through ``job_lock``.
    The directory is also unique per extraction generation, so an old cleanup
    thread can never target a replacement extraction's files.
    """
    ...
    with job_lock:
        current = extractions.get(meta.get("book_id"))
        same_generation = bool(current and current.get("meta") is meta)
        if same_generation:
            # The directory is unique to this generation. Delete it while the
            # identity check is held so a replacement cannot be installed
            # between the check and deletion.
            shutil.rmtree(chunk_dir, ignore_errors=True)
            logger.info("Cleaned temp dir: %s", chunk_dir)
            return

    # This is an old, uniquely-named generation. It is safe to reclaim after
    # releasing the lock because a replacement can never share its path.
    shutil.rmtree(chunk_dir, ignore_errors=True)
    logger.info("Cleaned stale temp dir: %s", chunk_dir)
...
def start_extraction(payload: dict):
    ...
    chunk_dir = _new_extraction_chunk_dir(base_dir, book_id)
    ...
    for chunk_idx, start in enumerate(range(0, total_pages, CHUNK_SIZE)):
        ...
        jobs[jid]["ocr_enabled"] = ocr_enabled
...
def get_result(book_id: str):
    ...
    engines = set()
    cumulative_pages = 0
    # Dispatch order may rotate retries, but assembled extraction must always
    # follow the original PDF chunk order.
    result_jobs = sorted(
        (state["jobs"][jid] for jid in state["queue"]),
        key=lambda job: (job.get("chunk_idx") is None, job.get("chunk_idx", 0)),
    )
    for job in result_jobs:
        result = job["result"] or {}
        if isinstance(result, str):
            result = {"markdown": result, "elements": [], "engine": "legacy"}
        md = result.get("markdown", "")
        page_count = len(re.findall(r"^## --- PAGE (\d+) ---", md, re.MULTILINE))
        md = re.sub(
            r"^## --- PAGE (\d+) ---",
            lambda m, offset=cumulative_pages: f"## --- PAGE {offset + int(m.group(1))} ---",
            md,
            flags=re.MULTILINE,
        )
