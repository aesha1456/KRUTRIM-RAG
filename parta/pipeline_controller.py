...
PHASE 1  (expensive, disk-checkpointed)
  Steps : Validate → Extract (OpenDataLoader hybrid) → Chunk → Triple-Rep → Propositions
...
from parta.config import EXTRACTION_SERVER_URL
...
def _run_qdrant_stage(job_id, book_id, ready_path, prop_path, jobs_col):
    ...
    try:
        ...
        while True:
            ...
            # Feed live worker progress back to MongoDB so the UI bar actually moves
            done = status_resp.get("completed", 0)
            total = status_resp.get("total", 0)
            live_pct = status_resp.get("percent")
            if live_pct is None and total:
                live_pct = int(done / total * 100)
                ...
                jobs_col.update_one(
                    {"job_id": job_id},
                    {
                        "$set": {
                            "qdrant_progress": {
                                "status": "running",
                                "percent": min(int(live_pct), 99),
                                "message": f"Embedding vectors... {done}/{total} batches done",
                            },
                            "updated_at": _now_iso(),
                        }
                    },
                    ...
                    {"job_id": job_id},
                    {
                        "$set": {
                            "neo4j_progress": {
                                "status": "running",
                                "percent": min(int(live_pct), 99),
                                "message": f"Building graph... {done}/{total} batches done",
                            },
                            "updated_at": _now_iso(),
                        }
                    },
...
@time_it
def _write_structured_extraction(
    processed_dir: Path,
    book_id: str,
    extraction_content: dict,
) -> Path:
    """Persist structured worker output for chunking and source highlights."""
    elements = extraction_content.get("elements", [])
    if not isinstance(elements, list):
        elements = []
    structured_file = processed_dir / f"{book_id}.json"
    structured_file.write_text(
        json.dumps(
            {
                "book_id": book_id,
                "engine": extraction_content.get("engines") or extraction_content.get("engine"),
                "elements": elements,
            },
            ensure_ascii=False,
            indent=2,
...
def run_phase1(job: dict, jobs_col) -> tuple:
    ...
    try:
        ...
        callback(
            percent=10,
            stage="Text Extraction",
            message=f"Queued {total_chunks} chunks ({total_pages} pages). Workers processing...",
        )
        ...
        while True:
            ...
            callback(
                percent=ui_pct,
                stage="Text Extraction",
                message=f"Extracted {completed} of {total_chunks} chunks",
            )

        # ── Step 3: Fetch assembled markdown ──────────────────────────
        callback(
            percent=49,
            stage="Text Extraction",
        ...
        if result_resp.status_code != 200:
            raise RuntimeError(f"Result fetch failed: {result_resp.status_code}")

        extraction_content = result_resp.json().get("content", {})
        full_text = extraction_content.get("markdown", "")
        extracted_elements = extraction_content.get("elements", [])
        ...
        # Keep the structured extraction beside Markdown. OCR elements carry
        # page numbers, native Docling references, and bounding boxes; losing
        # this JSON here would force chunk.py to rebuild chunks from Markdown
        # and silently discard all OCR geometry.
        structured_file = _write_structured_extraction(
            processed_dir, book_id, extraction_content
        )
        logger.info(
            "Saved structured extraction → %s (%d elements)",
            structured_file,
...
def run_phase2(job: dict, jobs_col, mongo_db):
    ...
    try:
        ...
        # Total processing time = start of Phase 1 → completion
        job_start = jobs_col.find_one({"job_id": job_id}, {"_id": 0, "started_at": 1})
        processing_seconds = None
        start_iso = (job_start or {}).get("started_at")
        if start_iso:
            try:
                processing_seconds = max(
                    0,
                    int((datetime.now(timezone.utc) - datetime.fromisoformat(start_iso)).total_seconds()),
                )
        ...
        library_col.update_one(
            {"book_id": book_id},
            {
                "$set": {
                    ...
                    "uploaded_by": user_id,
                    "uploaded_by_name": job_doc.get("uploaded_by_name", ""),
                    "status": "ready",
